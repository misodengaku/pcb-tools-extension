...
def loads(data, filename=None):
    ...
    cls.SF = r"(?P<param>SF)(A(?P<a>{decimal}))?(B(?P<b>{decimal}))?".format(
        decimal=cls.DECIMAL
    )
    cls.PARAMS = (
        cls.FS,
...
def write_gerber_header(file, settings):
    file.write(
        ...
        % (
            ...
            FSParamStmt(
                "FS", settings.zero_suppression, settings.notation, settings.format
            ).to_gerber(settings),
        )
    )
...
class GerberFile(gerber.rs274x.GerberFile):
    ...
    def from_gerber_file(cls, gerber_file):
        ...
        return cls(
            gerber_file.statements,
            gerber_file.settings,
            gerber_file.primitives,
            gerber_file.apertures,
            gerber_file.filename,
        )

    def __init__(self, statements, settings, primitives, apertures, filename=None):
        super(GerberFile, self).__init__(
            statements, settings, primitives, apertures, filename
        )
    ...
    def rotate(self, angle, center=(0, 0)):
        ...
        for statement in self.main_statements:
            if (
                isinstance(statement, CoordStmt)
                and statement.x != None
                and statement.y != None
            ):
    ...
    def _generalize_aperture(self):
        ...
        for statement in self.aperture_defs:
            if isinstance(statement, ADParamStmt) and statement.shape in [
                "R",
                "O",
                "P",
            ]:
            ...
            for statement in self.aperture_defs:
                if isinstance(statement, ADParamStmt):
                    if statement.shape == "R":
                        ...
                        x = (
                            statement.modifiers[0][0]
                            if len(statement.modifiers[0]) > 0
                            else 0
                        )
                        y = (
                            statement.modifiers[0][1]
                            if len(statement.modifiers[0]) > 1
                            else 0
                        )
                        statement.shape = (
                            macro_defs[LANDSCAPE_OBROUND][0]
                            if x > y
                            else macro_defs[PORTRATE_OBROUND][0]
                        )
...
class GerberContext(FileSettings):
    ...
    def __init__(
        ...
        super(GerberContext, self).__init__(
            notation, units, zero_suppression, format, zeros, angle_units
        )
        self.name = name
        self.mirror = mirror
        self.offset = offset
